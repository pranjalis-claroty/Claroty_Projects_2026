...
# =============================================================================
# 1. AUDIT: Remote CTD Server System Health Dashboard (DYNAMIC FIX)
# =============================================================================
print("[*] Fetching Metric: Remote System Health Dashboard...")
try:
    # Removed hardcoded site_id param so it gracefully queries the active host
    ...
    # Initialize empty defaults so we don't accidentally print fake data
    cpu_pct = "N/A"
    ram_pct = "N/A"
    partitions_summary = {}
    ...
    if health_res.status_code == 200:
        ...
        # The factors array is nested inside a dynamic key (UUID or Site Name).
        # We loop through the data block to find the first object containing 'factors'.
        factors = []
        ...
        if system_factor:
            ...
            raw_cpu = info_block.get("cpu", {}).get("value")
            raw_ram = info_block.get("memory", {}).get("value")
            
            if raw_cpu is not None: cpu_pct = f"{raw_cpu}%"
            if raw_ram is not None: ram_pct = f"{raw_ram}%"

            # --- RECOMMENDATION ENGINE: Memory Load ---
            if isinstance(raw_ram, (int, float)) and raw_ram > 85:
            ...
            # Dynamically build partitions so it matches 3 or 5 depending on the instance
            ...
            if isinstance(raw_ram, (int, float)) and isinstance(raw_cpu, (int, float)):
                assessment_status = "PASS" if (raw_ram < 85 and raw_cpu < 85) else "REVIEW"

    if not partitions_summary:
        partitions_summary = {"Error": "No storage partitions found in API response."}
...
try:
    ...
    # --- RECOMMENDATION ENGINE: Cleartext Protocol Check ---
    if proto_data.get("success"):
        insecure_protocols = ["telnet", "ftp", "rlogin", "pop", "imap", "ldap"]
        enabled_insecure = []
        ...
        if enabled_insecure:
            ...
            actionable_insights.append(f"🚨 INSECURE PROTOCOLS: Cleartext protocols detected as enabled ({proto_list_str}). In Federal zero-trust environments, these should be disabled unless actively required for legacy OT communications.")
    # -------------------------------------------------------
